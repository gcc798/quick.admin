...
func (r *Resources) issueSession(ctx context.Context, userID int64, client *AuthClientInfo, reuseExisting bool) (string, string, error) {
	if r == nil || r.Redis == nil || r.JWT == nil {
		return "", "", fmt.Errorf("session dependencies are not configured")
	}
	clientID := ""
	deviceType := ""
	if client != nil {
		clientID = strings.TrimSpace(client.ClientID)
		deviceType = strings.TrimSpace(client.DeviceType)
	}
	...
	userName := ""
	if user, err := r.GetUser(ctx, userID); err == nil && user != nil {
	...
	accessToken, _, err := r.JWT.GenerateToken(userID, userName, clientID, deviceType, accessTTL)
	if err != nil {
		return "", "", err
	}
...
func (r *Resources) RefreshSession(ctx context.Context, refreshToken string, client *AuthClientInfo) (int64, string, string, error) {
	if r == nil || r.Redis == nil || r.JWT == nil {
		return 0, "", "", fmt.Errorf("session dependencies are not configured")
	}
...
func (r *Resources) SessionDetails(ctx context.Context, accessToken string) (*sessionRecord, error) {
	if r == nil || r.Redis == nil || r.JWT == nil {
		return nil, fmt.Errorf("session dependencies are not configured")
	}
	accessToken = strings.TrimSpace(accessToken)
	if accessToken == "" {
		return nil, fmt.Errorf("access token is required")
	}
	claims, err := r.JWT.ValidateToken(accessToken)
	if err != nil {
	...
	if claims.UserID != record.UserID || strings.TrimSpace(claims.ClientID) != record.ClientID {
		_ = r.RevokeSession(ctx, accessToken, "")
		return nil, fmt.Errorf("invalid access token")
	}
	return &record, nil
}
