...
func NewStorageEnvRepo(endpoint string) (*StorageEnvRepo, error) {
	conn, err := dialRPC(endpoint)
	if err != nil {
		return nil, err
	}
	return &StorageEnvRepo{conn: conn, client: v1.NewStorageEnvServiceClient(conn)}, nil
}
func (r *StorageEnvRepo) Close() error {
	if r == nil || r.conn == nil {
		return nil
	}
	return r.conn.Close()
