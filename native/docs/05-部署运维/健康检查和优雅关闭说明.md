...
# prometheus.yml
scrape_configs:
  - job_name: 'quick.admin-health'
    metrics_path: '/health'
    scrape_interval: 30s
    static_configs:
      - targets: ['quick.admin-api:8080']
```
...
# alert-rules.yml
groups:
  - name: quick.admin-health
    rules:
      - alert: ServiceUnhealthy
        expr: up{job="quick.admin-health"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
...
```bash
# 构建镜像
docker build -t quick.admin:test .

# 运行容器
docker run -p 8080:8080 quick.admin:test
