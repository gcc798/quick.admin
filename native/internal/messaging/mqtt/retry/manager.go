...
import (
	...
	"github.com/force-c/nai-tizi/internal/messaging/mqtt/protocol"
	"github.com/force-c/nai-tizi/internal/utils"
	"github.com/force-c/nai-tizi/pkg/mqtt"
	redisutils "github.com/force-c/nai-tizi/pkg/redis"
...
func NewManager(db *gorm.DB, mqttClient *mqtt.Client, redisClient *goredis.Client, logger logging.Logger, config *RetryConfig) *Manager {
	...
	return &Manager{
		...
		logger:     logger,
		redisUtils: redisutils.NewRedisUtils(redisClient),
		config:     config,
		ctx:        ctx,
		cancelFunc: cancel,
	}
...
func (m *Manager) SubmitRetryMessage(request *RetryMessageRequest) (int64, error) {
	...
	if err := m.sendMqttMessage(request); err != nil {
		...
		return 0, fmt.Errorf("发送MQTT消息失败: %w", err)
	}
...
func (m *Manager) sendMqttMessage(request *RetryMessageRequest) error {
	netType := request.NetType
	if netType == "" {
		netType = protocol.NetTypeWiFi
	}
	topic := protocol.BuildTopic(netType, request.DeviceMac, request.DeviceSnNum)
