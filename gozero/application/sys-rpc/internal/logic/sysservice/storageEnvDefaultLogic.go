...
func (l *StorageEnvDefaultLogic) StorageEnvDefault(in *pb.Empty) (*pb.StorageEnv, error) {
	...
	if err := l.svcCtx.DB.QueryRowCtx(l.ctx, &row, `select id, name, code, storage_type, is_default, status, config, remark, create_by, created_time, update_by, updated_time from public.s_storage_env where is_default = true and status = 0 and deleted_at is null order by id desc limit 1`); err != nil {
		return nil, err
	}
